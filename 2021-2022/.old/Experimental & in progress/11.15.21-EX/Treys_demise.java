...
double rightPower;
    double servoPower = 1;
        ...
        double power = 1;
        double poweralt = 1;
    if(gamepad1.mode_button){
        boolean Tmode = True 
        Telemetry.addData("Status","Tmode active")
    } else {
        boolean Tmode = false
        Telemetry.addData("Status","Tmode disabled")
    } 
  
    //Chill Creates a slow Driveing mode 
   
    //IF right triger is pressed THEN set power to 2 (slow)
    // Tmode must be false for this to work 
    if (Tmode = false) {
...
//DRIVE CONTROLS
 ...
 if (Tmode = false) {
     ...
     //When tmode is active I mess with the numbers alot  
      if else (Tmode = true) {
    leftPower    = Range.clip(drive + turn, -.20/power, .57/poweralt) ;
       rightPower   = Range.clip(drive + turn, -.57/power, .40/poweralt) ;
      } 
//SPINNER
if (Tmode = false)
